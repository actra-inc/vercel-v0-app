...
export function WorkLogPanel({
    ...
    async (blob: Blob, opts?: { force?: boolean; screenCount?: number }) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          // AI応答のJSONが解釈できなかった（この回は保存されない）→ 次回に任せる
          if (errorData.error === "parse_failed") {
            console.warn("⚠️ AI response could not be parsed; skipping this capture")
        ...
        const errorMessage = error instanceof Error ? error.message : t('wlp_unknown')
        const isAbort =
  ...
  // confirm()/alert() はモーダルで JS スレッドを止め、解析ループのタイマーまで
  // 止めてしまう（表示中はキャプチャが発火しない）ため、AlertDialog とインライン通知に置き換えている
  ...
  const performClearLogs = async () => {
    ...
    if (result && result.dbDeleteFailed) {
      setLastAnalysisError(t('rt_deleteNoPermission'))
    } else {
      setFeedbackMessage(t('wlp_clearAllDone'))
      setTimeout(() => setFeedbackMessage(null), 5000)
    }
  }
  const onClearLogs = () => setClearDialogOpen(true)
  ...
  return (
    <div className="space-y-4">
      ...
      <AlertDialog open={clearDialogOpen} onOpenChange={setClearDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('wlp_clearAllConfirm')}</AlertDialogTitle>
            <AlertDialogDescription>{t('wlp_clearAllDesc')}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('common_cancel')}</AlertDialogCancel>
            <AlertDialogAction onClick={performClearLogs} className="bg-red-600 hover:bg-red-700">
              {t('wlp_clearAll')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
            ...
            <div className="space-y-4 pb-4">
              ...
              {regularLogsCount === 0 && (
                <div className="text-center py-8 text-gray-500">
                  {t('wlp_noLogsYet')}
                  <br />
                  {t('wlp_noLogsYetHint', { start: t('wlp_startAnalysis'), screenshot: t('wlp_screenshot') })}
                </div>
              )}
            </div>
          </ScrollArea>
